...
watch(
    ...
    (f) => {
        revoke()
        if (!f) return
        if (f instanceof File) fileName.value = f.name
        else if (typeof f === 'string') fileName.value = f
    },
    { immediate: true }
)
...
function validHtml(file) {
    ...
    const byExt  = (file.name || '').toLowerCase().endsWith('.html')
...
<template>
    <label
        ...
        @dragleave="onDrag"
    >
        <input type="file" accept=".html,text/html" hidden @input="onInput" />
