...
const rejectUrl = computed(() => {
    return route('admin.moderation.hackathonsreject', { hackathon: props.hackathonSlug })
})
...
async function submitReject() {
    ...
    try {
        ...
        await axios.post(rejectUrl.value, payload)
        toast.success(capitalizeFirstLetter(langStore.translations.hackathon_cancelled_success), { position:'top-right', timeout:5000 })
        close()
    } catch (e) {
        ...
        if (e?.response?.status === 422) {
            ...
            toast.error(capitalizeFirstLetter(langStore.translations.hackathon_cancel_failed), { position:'top-right', timeout:5000 })
        }
    } finally {
        pending.value = false
    }
    ...
    <div v-if="modelValue" class="dialog" style="z-index:2">
        <div class="dialog__container dialog__container_small" @click.stop>
            <div class="dialog__header">
                <p>{{ capitalizeFirstLetter(langStore.translations.leave_comment) }}</p>
            ...
            <div class="dialog__component" style="margin-top: -10px">
                ...
                <textarea
                    ...
                    :placeholder="capitalizeFirstLetter(langStore.translations.enter_comment)"
