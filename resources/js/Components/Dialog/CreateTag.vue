...
const title = computed(() => (isEdit.value ? 'Изменить тег' : 'Добавить тег'))
const submitLabel = computed(() => (isEdit.value ? 'Сохранить' : 'Добавить'))

const disabled = computed(() => {
    const v = (nameTag.value || '').trim()
    return pending.value || v.length < 2 || v.length > 50
})
...
function submit () {
    ...
    const v = (nameTag.value || '').trim()
    const onSuccess = () => {
        toast.success(isEdit.value ? 'Тег успешно обновлён' : 'Тег успешно добавлен', {
            position: 'top-right',
            timeout: 4000,
        })
    ...
    const onError = (err) => {
        ...
        toast.error(first || 'Не удалось сохранить тег', { position: 'top-right', timeout: 5000 })
    }
    const onFinish = () => { pending.value = false }
    if (isEdit.value) {
        router.post(
            route('admin.contents.tagsupdate', props.tag.slug),
            { _method: 'patch', title: v },
            { preserveState: true, preserveScroll: true, replace: true, onSuccess, onError, onFinish }
        )
    } else {
        router.post(
            route('admin.contents.tagsstore'),
            { title: v },
            { preserveState: true, preserveScroll: true, replace: true, onSuccess, onError, onFinish }
        )
    }
    ...
    <div v-if="modelValue" class="dialog" style="z-index: 2">
        <div class="dialog__container dialog__container_small" @click.stop>
            ...
            <div class="dialog__component">
                ...
                <input
                    v-model.trim="nameTag"
                    type="text"
                    class="dialog__input"
                    placeholder="Введите название тега"
                    @keyup.enter="submit"
                >
                <p v-if="errors.title" class="dialog__error" style="margin-top:8px">{{ errors.title }}</p>
                <p class="dialog__hint" style="margin-top:6px; color:#999">От 2 до 50 символов</p>
            </div>

            <div class="dialog__component" v-if="nameTag">
                <p class="dialog__title">Предпросмотр</p>
            ...
            <div class="dialog__btns">
                ...
                <button class="main__btn dialog__btn" :class="{ blocked: disabled }" :disabled="disabled" @click="submit">
                    {{ submitLabel }}
                </button>
            </div>
        </div>
...
<style scoped>
.dialog__error{ color:#E80024; font-size:14px; line-height:1.3 }
</style>
