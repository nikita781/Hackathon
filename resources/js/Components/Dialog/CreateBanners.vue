...
watch(
    ...
    (open) => {
        ...
        image.value = props.banner?.image_path ?? null;
        errors.value = {};
    }
);
...
function submit() {
    ...
    router.post(
        isEdit.value
            ? route("admin.contents.bannersupdate", props.banner.id)
            : route("admin.contents.bannersstore"),
        data,
        {
            preserveState: true,
            preserveScroll: true,
            replace: true,
            onSuccess: () => {
                toast.success(isEdit.value ? "Баннер успешно обновлен" : "Баннер успешно добавлен", {
                    position: "top-right",
                    timeout: 4000,
                });
                emit("saved");
            ...
            onError: (err) => {
                errors.value = err || {};
                const first = err && (err.image || Object.values(err)[0]);
                toast.error(first || "Не удалось сохранить баннер", { position: "top-right", timeout: 5000 });
            },
