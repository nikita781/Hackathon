...
const isEdit = computed(() => !!props.award);
...
function submit() {
    ...
    router.post(
        ...
        data,
        {
            ...
            replace: true,
            onSuccess: () => {
                toast.success("Изображение награды успешно обновлено", {position: "top-right", timeout: 4000});
                emit("saved");
                emit("update:modelValue", false);
            },
            onError: (err) => {
                ...
                toast.error(first || "Не удалось сохранить изображение", {position: "top-right", timeout: 5000});
            },
            onFinish: () => {
                pending.value = false;
            },
    ...
    <div v-if="modelValue" class="dialog" style="z-index: 2">
        <div class="dialog__container dialog__container_small" @click.stop>
            ...
            <div class="dialog__component">
                ...
                <DropFile v-model:file="image" accept="image/jpeg,image/png,image/webp"/>
                <small v-if="errors.image" class="dialog__error" style="margin-top:8px">{{ errors.image }}</small>
                <p class="dialog__hint" style="margin-top:6px; color:#999">JPEG/PNG/WebP, до 2 МБ</p>
            </div>

            <div class="dialog__btns">
                <button class="main__btn main__btn_white dialog__btn" @click="close">Отменить</button>
                <button class="main__btn dialog__btn" :class="{ blocked: disabled }" :disabled="disabled"
                        @click="submit">
                    {{ submitLabel }}
                </button>
            </div>
