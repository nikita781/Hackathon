...
const props = defineProps({
    modelValue: Boolean,
    managers  : { type: Array,  default: () => [] },   // было Object
    hackathon : { type: Object, required: true },      // было Array
})

const emit = defineEmits([
    'update:modelValue',
    'update:managers',   // чтобы родитель мог обновиться
    'removed',           // опционально: кто был удалён
])
...
// локальный список для отображения (не мутируем props)
const list = ref([])
watch(
    () => props.managers,
    ...
    <div v-if="modelValue" class="dialog" style="z-index:2">
        <div class="dialog__container dialog__container_small" @click.stop>
            ...
            >
                ...
                <select v-model="person.hackathon_role.id" class="main__cards_select dialog__select" style="width: 100%; max-width: 230px" disabled>
                    <option v-for="p in rolesResp.roles" :key="p.id" :value="p.id">{{ p.title }}</option>
                </select>
            ...
            <div class="dialog__btns">
                <button class="main__btn main__btn_white dialog__btn" @click="close">Закрыть</button>
            </div>
        </div>
    </div>
</template>
