...
import TagDialog from "@/Components/Dialog/CreateTag.vue";
...
function saveOrder() {
    ...
    router.post(
        route("admin.contents.tagschange-order"),
        payload,
        {
            preserveState: true,
            preserveScroll: true,
            replace: true,
            onSuccess: () => {
                toast.success("Порядок тегов успешно изменён", { position: "top-right", timeout: 4000 });
            },
            onError: (err) => {
...
function deleteTag(row) {
    ...
    router.post(
        route('admin.contents.tagsdelete', row.slug),
        { _method: 'delete' },
        {
            preserveState: true,
            preserveScroll: true,
            replace: true,
            onSuccess: () => {
                toast.success('Тег успешно удален', { position: 'top-right', timeout: 4000 })
            },
            onError: () => {
                toast.error('Не удалось удалить тег', { position: 'top-right', timeout: 5000 })
            },
            onFinish: () => {
            ...
            <table class="admin__table admin__table--draggable" style="margin-top: 30px;">
                ...
                >
                    ...
                    <td style="text-align:end; padding: 14px 16px 14px 5px">
                        <button class="btn-drop" aria-label="Удалить" title="Удалить" @click.stop="deleteTag(u)">
