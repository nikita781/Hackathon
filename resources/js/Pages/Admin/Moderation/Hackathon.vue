...
function setActiveTab(i){ activeTab.value = i }
...
const order  = ref(props.filters.order ?? 'createdD') // «По дате создания»
const selected = ref({
...
function runSearch() {
    // имя роута у тебя: admin.moderation.hackathonsindex
    const base = route('admin.moderation.hackathonsindex')
...
// --- пагинация (поддерживаем и paginator.links[], и resource.meta.links)
const withQ = (url) => {
    if (!url) return null
    const q = buildQuery()
    const usp = new URLSearchParams()
    Object.entries(q).forEach(([k,v]) => { if (v != null && v !== '') usp.set(k, v) })
...
onMounted(async () => {
    ...
    await nextTick(() => {
        tabEls.value = Array.from(document.querySelectorAll<HTMLElement>('.my-hackathon__tabs_item'))
    })
...
<template>
    <AuthenticatedLayout>
        ...
        <div class="admin__content">
            ...
            <div class="my-hackathon__tabs">
                <p :ref="setTabRef(0)"
                   ...
                   @click="setActiveTab(0)">Хакатоны</p>

                <p :ref="setTabRef(1)"
                   :class="['my-hackathon__tabs_item',{active:activeTab===1}]"
                   @click="setActiveTab(1)">Проекты</p>
            ...
            <div class="main__cards" style="margin-top: 40px">
                <a v-for="hackathon in rows" :key="hackathon.id" class="main__card" :href="`/hackathons/${hackathon.slug}`">
                    ...
                    <div class="main__card_content">
                        ...
                        <div class="main__card_info" v-if="activeTab === 0">
        ...
        <FiltersMenu
            ...
            @reset="() => { order='createdD'; selected={status:[]}; search=''; runSearch() }"
        />
    </AuthenticatedLayout>
</template>
