...
function goTab(i) {
    ...
    if (i === 0) {
        router.get(route('admin.moderation.hackathonsindex'), params, {
            preserveState: true,
            preserveScroll: true,
            replace: true
        })
...
function runSearch() {
    router.get(route('admin.moderation.hackathonsindex'), buildQuery(), {
        preserveState: true,
        preserveScroll: true,
        replace: true
    })
...
async function finishHackathon() {
    ...
    syncing.value = true
    try {
        await axios.post(route('admin.hackathons.finish'))
        await router.reload({ only: ['hackathons'], preserveState: true, preserveScroll: true })
        toast.success('Хакатоны успешно завершены!', {
            position: 'top-right',
            timeout: 5000,
        })
    } catch (e) {
        console.error('Ошибка при завершении хакатонов', e?.response ?? e)
        toast.error('Ошибка при завершении хакатонов.', {
            position: 'top-right',
            timeout: 5000,
        })
    } finally {
    ...
    >
        ...
        <div class="admin__content">
            ...
            <button
                ...
                :disabled="syncing"
            >
                {{ syncing ? 'Завершаем…' : 'Завершить хакатоны' }}
            </button>
            ...
            <!-- Карточки: контент ПЕРВОГО таба (как было у тебя при activeTab===0) -->
            <div class="main__cards" style="margin-top: 40px">
                <a v-for="hackathon in rows" :key="hackathon.id" class="main__card"
                        ...
                        <div class="main__card_info">
                            <div class="main__card_info-long">
                                ...
                                <div class="main__card_item">
                                    <div style="width:24px;height:24px"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <p>
                                        {{
                                            hackathon.format === 'online'
                                                ? capitalizeFirstLetter(langStore.translations.online)
                                                : hackathon.format === 'offline'
                                                    ? capitalizeFirstLetter(langStore.translations.offline)
                                                    : capitalizeFirstLetter(langStore.translations.hybrid)
                                        }}
                                    </p>
                                </div>

                                <!-- тип команды/индивидуально -->
                                <div class="main__card_item">
                                    <div style="width:24px;height:24px"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
